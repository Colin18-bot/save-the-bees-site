...
async function safeSendTemplate(
  ...
  if (!BREVO_API_KEY) {
    console.error(
      `📧 ${label} not sent: BREVO_API_KEY is missing.`,
    );
  ...
  if (validRecipients.length === 0) {
    console.error(
      `📧 ${label} not sent: no valid recipient email.`,
    );
...
async function claimStripeEvent(
  ...
  if (error.code === "23505") {
    console.log(
      `ℹ️ Stripe event ${event.id} has already been processed.`,
    );
...
async function releaseStripeEvent(
  ...
  if (error) {
    console.error(
      `Unable to release Stripe event ${eventId}:`,
      error.message,
    );
  }
}
...
async function handlePremiumCheckout(
  ...
  await safeSendTemplate(
    ...
    {
      templateId:
        EMAIL_TEMPLATES.PREMIUM_WELCOME,
      to: {
        email: customerEmail,
        name: customerName,
      },
  ...
  await safeSendTemplate(
    "Internal New Premium Subscriber",
    {
      templateId:
        EMAIL_TEMPLATES
          .INTERNAL_NEW_PREMIUM_SUBSCRIBER,
      to: {
...
async function handleNfcCheckout(
  ...
  const quantity =
    quantityFromMetadata > 0
      ? quantityFromMetadata
  ...
  await safeSendTemplate(
    "Internal NFC Order Notification",
    {
      templateId:
        EMAIL_TEMPLATES
          .NFC_INTERNAL_ORDER_NOTIFICATION,
      to: {
        email: INTERNAL_EMAIL,
        name: "BeezKnees Support",
      },
...
async function handleSubscriptionSync(
  ...
  if (!customerId) {
    console.warn(
      `⚠️ Subscription ${subscription.id} has no customer ID.`,
    );
    return;
  }
...
async function handleSubscriptionDeleted(
  ...
  if (!customerId) {
    console.warn(
      `⚠️ Deleted subscription ${subscription.id} has no customer ID.`,
    );
    return;
  }
  ...
  await safeSendTemplate(
    "Internal Subscription Cancelled",
    {
      templateId:
        EMAIL_TEMPLATES
          .INTERNAL_SUBSCRIPTION_CANCELLED,
      to: {
        email: INTERNAL_EMAIL,
        name: "BeezKnees Support",
      },
...
async function handleInvoicePaid(
  ...
  if (
    ...
    console.log(
      "ℹ️ Paid invoice is not linked to a subscription.",
    );
    return;
  }
  ...
  if (
    ...
    console.log(
      `ℹ️ Renewal email skipped for billing_reason=${invoice.billing_reason}.`,
    );
    return;
  }
...
async function handleInvoicePaymentFailed(
  ...
  if (!customerId) {
    console.warn(
      `⚠️ Failed invoice ${invoice.id} has no customer ID.`,
    );
    return;
  }
...
serve(async (
  ...
  console.log(
    `🔔 Received Stripe event ${event.type} (${event.id}).`,
  );
  ...
  try {
    ...
    switch (event.type) {
      ...
      default: {
        console.log(
          `ℹ️ No HiveTag handler is required for ${event.type}.`,
        );
      }
    }
    ...
    console.error(
      `❌ Handler error for ${event.type} (${event.id}):`,
      message,
    );
