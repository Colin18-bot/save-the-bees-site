...
const InspectionList = () => {
  ...
  // NEW: related logbook map { [inspection_id]: { count, recent: Log[] } }
  ...
  // --- Read URL params for filters (ignore highlight/type here)
  useEffect(() => {
    const params = new URLSearchParams(location.search || "");
  ...
  // --- Push current filters to URL (preserve highlight & type if present)
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchInspections = async () => {
      ...
      if (!error) {
        ...
        // NEW: batch fetch related logbook entries for these inspections
  ...
  // --- Auto-scroll to the highlighted card once inspections are loaded
  useEffect(() => {
    ...
    if (el) {
      ...
      // Optional: brief flash animation class
  ...
  // 🔧 SAFE DATE FORMATTER (prefers date, falls back to created_at; never uses "today" for undefined)
  ...
  const buildSummary = (insp) => {
    ...
    // Disease/Pests handled separately for styling
    if (insp.signs_pests) {
  ...
  // Disease helper: decide highlight
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            {inspections.map((insp) => {
              ...
              const isHighlighted =
                highlightId && String(insp.id) === String(highlightId) &&
              ...
              return (
                ...
                >
                  ...
                  {/* Optional tiny previews of up to 2 recent log entries */}
            ...
            <div className="flex gap-2">
              ...
              {Math.max(1, Math.ceil(total / PAGE_SIZE)) > 1 && (
                <div className="flex gap-1">
                  {Array.from({ length: Math.max(1, Math.ceil(total / PAGE_SIZE)) }, (_, i) => i + 1).map((n) => (
                    <button
                      key={n}
                      type="button"
                      onClick={() => setPage(n)}
