...
export default function NewSale() {
  ...
  // currency from Settings (fallback GBP)
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // Normalise lines:
      // - Auto-generate name if Product is blank: `${Type} (${Unit})`
      // - Keep only lines with Qty > 0
      ...
      if (!normalised.length) throw new Error("Add at least one line item.");

      // Order header – use date string (Postgres will treat 'YYYY-MM-DD' as midnight)
      const order = {
        user_id: uid,
      ...
      const { data: ins, error: oErr } = await supabase
        .from("sales_orders")
        .insert([order])
        .select("id")
        .single();
      ...
      const rows = normalised.map(l => ({
        ...
        discount: l.discount,
        cogs_per_unit_cached: l.cogs_per_unit_cached,
      }));

      const { error: lErr } = await supabase.from("sales_lines").insert(rows);
      if (lErr) throw lErr;
